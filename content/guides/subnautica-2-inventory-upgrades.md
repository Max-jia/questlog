---
title: "Subnautica 2 Inventory & Hotbar Upgrades : How to Carry More and Access Faster"
excerpt: "Your starting inventory is tiny. Here is how to upgrade it, get more hotbar slots, and manage your loadout so you stop dropping things on the ocean floor."
game: subnautica-2
gameName: "Subnautica 2"
date: "2026-07-13"
category: Guides
section: Beginner
verified: verified
gameVersion: "Early Access (May 2026)"
tldr:
  - "The Endurance Adaptation adds inventory and hotbar slots. Find it at Sophie's Retreat, Cicada Wreck, and the Unauthorized NOA Mod."
  - "Dexterity improves tool handling speed. Get it at Nahema's Lab in the Hydrothermal Vents."
  - "Drop items on the ground to trade with co-op partners. There is no direct trade menu."
tags:
  - Inventory
  - Hotbar
  - Upgrades
  - Beginner
sources:
  - name: "Wikily : Subnautica 2 FAQ"
    url: "https://wikily.gg/subnautica-2/faq/"
  - name: "GameRant : Beginner Tips"
    url: "https://gamerant.com/subnautica-2-beginner-tips/"
---

![Subnautica 2 inventory upgrades](/images/subnautica-2/screenshots/screenshot-01.jpg)

You start with barely enough space to carry the basics. Two dives and your inventory is full of Titanium you might need later and fish you are not sure you can eat yet. Here is how to fix it.

---

### The Endurance Adaptation: more slots

Endurance adds inventory slots and quickbar slots. It comes from three locations, not one. You need to visit them all:

- Sophie's Retreat (early, easy find)
- Cicada Wreck (near The Hole, deeper)
- Unauthorized NOA Mod (mid-game)

Each visit grants additional slots. Do not stop after the first one. Go back for the other two when you have better gear.

---

### Dexterity: faster tools, more efficiency

Nahema's Lab in the Hydrothermal Vents. You need the Sonic Resonator to clear the path, and Heat Tolerance helps. Dexterity speeds up tool animations. Faster harvesting means more resources per dive, which means fewer trips.

---

### The personal storage module

The 1.1 Adaptive Measures update added a new buildable: a personal storage unit not connected to fabricators. Build one in your base. Drop items you want to keep separate from the crafting pool. No more accidental Titanium consumption because the fabricator auto-pulled from your main locker.

---

### Co-op inventory: just drop it

Character inventories are not shared in co-op. There is no trade window and no shared stash. To give something to a teammate, drop it on the ground. They pick it up. That is the system. Labeled lockers in your base are the closest thing to shared storage.

---

### What to carry on a standard expedition

Hotbar slots (1-5):
1. Survival Multitool
2. Scanner
3. Wakemaker
4. Sonic Resonator
5. Repair Tool (if using Tadpole) or Air Bladder

Inventory: 2 medkits, 2 spare batteries, 2 beacons, 2 water bottles, 2 cured fish. Leave half your inventory empty for resources. If you fill it on the way out, you filled it too early.

---

## Related Guides
- [Beginner's guide](/games/subnautica-2/subnautica-2-beginner-guide)
- [Best tools and equipment](/games/subnautica-2/subnautica-2-tools-equipment)
- [Adaptations and Angel Combs guide](/games/subnautica-2/subnautica-2-adaptations-guide)
