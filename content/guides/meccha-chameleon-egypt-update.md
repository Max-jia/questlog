---
title: "Meccha Chameleon Updates 2.6 & 2.7 — Egypt Map, HIKAKIN Museum & Clone Controversy"
excerpt: "Two major Meccha Chameleon updates dropped. Egypt and HIKAKIN Museum maps, random map toggles, Penguin Hotel randomization, and a clone game causing controversy."
game: meccha-chameleon
gameName: "Meccha Chameleon"
date: "2026-07-11"
category: News
verified: verified
gameVersion: "2.7.0"
tldr:
  - "Update 2.7.0 adds Egypt map and random map selection. Update 2.6.0 adds HIKAKIN Museum and alpha controller support"
  - "A Korean clone called Scribble Hunt is using Meccha Chameleon's name on the Korean Steam store"
  - "Penguin Hotel now randomizes furniture between rounds. FOV slider increased to 105."
tags:
  - Update
  - Patch Notes
  - Egypt
  - News
sources:
  - name: "ConsolePCGaming — Egypt Update"
    url: "https://consolepcgaming.com/meccha-chameleon-adds-egypt-and-more-random-map-options/"
  - name: "ConsolePCGaming — HIKAKIN Museum Update"
    url: "https://consolepcgaming.com/meccha-chameleon-update2-6-0-adds-a-hikakin-museum-collab-map/"
  - name: "VGtimes — Clone Controversy"
    url: "https://vgtimes.com/gaming-news/160888-a-clone-of-the-years-best-selling-game-snuck-onto-steam/"
---

![Meccha Chameleon Egypt update](/images/meccha-chameleon/screenshot-09.jpg)

The dev keeps shipping. Two updates landed in the past week, adding new maps, quality-of-life stuff, and alpha controller support. Meanwhile a Korean clone showed up on Steam using the exact same name. Here is everything.

---

## Update 2.7.0, Egypt map and random map toggles

### New map: Egypt
A desert-themed map with pyramids, ruins, and wide open spaces. Early impressions from the community suggest it is one of the harder hiding maps. Lots of flat surfaces, limited clutter. Hiders will need to lean on texture matching and lighting more than usual.

### Random map selection
You can now toggle which maps appear in your random rotation. Sick of getting Osaka every third game? Uncheck it. Want to grind the new Egypt map? Check only Egypt. This was a requested feature and it arrived fast.

### Penguin Hotel randomization
Furniture in Penguin Hotel now shuffles between rounds. The plushie that was your go-to hiding spot might be gone next round. This follows the Mansion randomization from the last patch. Good for game health, bad for people who memorized one spot and called it skill.

---

## Update 2.6.0, HIKAKIN Museum and alpha controller support

### HIKAKIN Museum collaboration map
A new map designed with HIKAKIN, a Japanese YouTuber with 19 million subscribers. The museum setting means lots of exhibit displays, glass cases, and curated spaces. Good variety from the usual house and city layouts.

### Alpha controller support
You can now play with a controller. It is labeled alpha for a reason. Expect bugs. Expect weirdness. But if you have been waiting for controller support to try the game, it is here in its first rough form.

### Other 2.6.0 changes
- FOV slider expanded from 100 to 105
- Region tags added for Japan, South Korea, and East Asia
- General bug fixes

---

## The Scribble Hunt clone controversy

A game called Scribble Hunt appeared on Steam and, on the Korean storefront, listed itself as "메챠 카멜레온", the exact Korean name for Meccha Chameleon. Korean players searching for the real game might buy the clone by mistake. It copies the paint-to-hide mechanic but adds prop hunt-style object transformation and gives hunters assault rifles and splat bombs.

No lawsuit has been filed. The Meccha Chameleon dev has not commented publicly. The community has been flagging it on Steam discussions.

---

## Community anti-cheat vigilantes

Separate from the clone situation, players are fighting cheaters in creative ways. Some cheaters use auto-paint scripts for instant perfect camouflage. The community response? Players paint visible arrows on the ground pointing at the cheater, sacrificing their own hiding spot to expose them. Petty. Beautiful. Effective.

---

## Related Guides
- [July patch roundup, 2.3.2 and earlier](/games/meccha-chameleon/meccha-chameleon-july-updates)
- [All maps and best hiding spots](/games/meccha-chameleon/meccha-chameleon-all-maps-guide)
- [Complete beginner's guide](/games/meccha-chameleon/meccha-chameleon-beginner-guide)
