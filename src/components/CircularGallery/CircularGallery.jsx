...
class App {
  ...
  onTouchDown(e) {
    ...
    // Remember where the press started so we can tell a click from a drag.
    this.pressStartX = this.start;
    this.pressStartY = e.touches ? e.touches[0].clientY : e.clientY;
    this.hasDragged = false;
  }
  onTouchMove(e) {
    if (!this.isDown) return;
    const x = e.touches ? e.touches[0].clientX : e.clientX;
    if (Math.abs(x - this.pressStartX) > 8) this.hasDragged = true;
  ...
  onTouchUp(e) {
    ...
    this.isDown = false;
    this.onCheck();
    // A press that barely moved counts as a click on the card underneath.
    if (wasDown && !this.hasDragged && e) {
      const point = e.changedTouches ? e.changedTouches[0] : e;
      if (point && typeof point.clientX === 'number') {
        this.handleClick(point.clientX, point.clientY);
      }
    }
  }
  handleClick(clientX, clientY) {
    ...
    // Convert the pixel position into the same coordinate space the planes
    // live in, then find the card whose horizontal span contains it.
    const worldX = ((clientX - rect.left) / rect.width - 0.5) * this.viewport.width;
    const hit = this.medias.find(media => Math.abs(media.plane.position.x - worldX) <= media.plane.scale.x / 2);
    if (hit) this.onItemClick({ image: hit.image, text: hit.text, link: hit.link });
  }
  onWheel(e) {
    // Scrolling the gallery shouldn't scroll the page underneath it.
    e.preventDefault();
  ...
  addEventListeners() {
    ...
    // Wheel and press events start on the gallery itself so the rest of the
    // page keeps scrolling normally; move/up stay on window so a drag that
    // leaves the gallery still finishes cleanly.
...
/**
 * @typedef {{ image: string, text: string, link?: string }} GalleryItem
 * @param {{
 *   items?: GalleryItem[],
 *   bend?: number,
 *   textColor?: string,
